...
Cada dimensión se convierte a un **score del 1 al 5** usando quintiles: - **5** = mejor comportamiento (compró recientemente / compra mucho / gasta mucho) - **1** = peor comportamiento
...
A partir del RFM se define: - `Es_Activo = "Activo"` → Recencia ≤ 90 días - `Es_Activo = "En_Riesgo"` → Recencia \> 90 días
...
Visualiza los 4 casos posibles de clasificación: - **Verdadero Positivo (VP):** cliente en riesgo correctamente detectado - **Verdadero Negativo (VN):** cliente activo correctamente identificado - **Falso Positivo (FP):** cliente activo incorrectamente marcado como en riesgo - **Falso Negativo (FN):** cliente en riesgo no detectado (el error más costoso)
